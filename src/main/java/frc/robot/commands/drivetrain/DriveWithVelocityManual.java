...
public class DriveWithVelocityManual extends IndefiniteCommand {
   ...
   public void initialize() {
      ...
      // Drivetrain.getInstance().getLeftMaster().selectProfileSlot(Drivetrain.VELOCITY_SLOT_INDEX,
      // Global.PID_PRIMARY);
      // Drivetrain.getInstance().getRightMaster().selectProfileSlot(Drivetrain.VELOCITY_SLOT_INDEX,
      // Global.PID_PRIMARY);
      // Drivetrain.getInstance().configBothFeedbackSensors(FeedbackDevice.CTRE_MagEncoder_Relative,
      // Global.PID_PRIMARY);
   ...
   public void execute() {
      ...
      SmartDashboard.putNumber("sensor health",
            Drivetrain.getInstance().getRightMaster().getSensorCollection().getPulseWidthPosition());
