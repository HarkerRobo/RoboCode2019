...
public class Wrist extends Subsystem {
   ...
   static {
      if (RobotMap.ROBOT_TYPE == RobotType.COMP) {
         ...
         SCORING_POSITION_BACK_HATCH_2 = 165;
         ...
         BACK_LOW_PASSTHROUGH_ANGLE = 179;
   ...
   /**
    * The percentage distance from either the front or the back after which
    * precautionary measures must be taken to limit max operable speed.
    */
   public static final double SLOW_DOWN_PERCENT_TO_ENDPOINT = 0.75;

   /*
    * The physical distance (encoder units) from either the front or the back after
    * which precautionary measures must be taken to limit max operable speed.
    */
   public static final double SLOW_DOWN_DISTANCE_FROM_ENDPOINT = Wrist.RANGE_OF_MOTION
         * (1 - Wrist.SLOW_DOWN_PERCENT_TO_ENDPOINT);
   ...
   public static Supplier<Double> feedForwardLambda = () -> (ARBITRARY_FF * Math.cos(
         Wrist.getInstance().convertEncoderToRadians(Wrist.getInstance().getMasterTalon().getSelectedSensorPosition())
               - WRIST_PARALLEL_FORWARD_DEG * Math.PI / 180));
   ...
   public void setWristPercentOutput(double value, WristDirection direction) {
      wristMaster.set(ControlMode.PercentOutput, value * direction.getSign(), DemandType.ArbitraryFeedForward,
            feedForwardLambda.get());
   }
   ...
   /**
    * Determines if the current position is behind the specified position
    * 
   ...
   /**
    * Determines if the current position is in front of the specified position.
    * 
   ...
   /**
    * Determines if the current position is in the ambiguous region.
    */
   ...
   public boolean mustPassThrough(int desiredWristPosition) {
      return Wrist.getInstance().isAmbiguous() || (Wrist.getInstance().isSafelyForward(desiredWristPosition)
            && Wrist.getInstance().isSafelyBackward()
